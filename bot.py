...
def _grad_bar(img, draw, x, y, w, h, r, c0, c1):
    """Horizontal gradient rounded bar."""
    if w <= 0:
        return
    BG = (18, 18, 30)
    for i in range(w):
        ...
        draw.rectangle([x + i, y, x + i, y + h], fill=col)
    r = max(0, min(r, w // 2, h // 2))
    if r < 1:
        return
    # Clip corners by painting BG over them with inverse-circle mask
    for (ox, oy), ellipse in [
        ((x,         y        ), [-1,    -1,    r*2-1, r*2-1]),
        ((x + w - r, y        ), [-r-1,  -1,    r-1,   r*2-1]),
        ((x,         y + h - r), [-1,    -r-1,  r*2-1, r-1  ]),
        ((x + w - r, y + h - r), [-r-1,  -r-1,  r-1,   r-1  ]),
    ]:
        cm = Image.new("L", (r, r), 255)
        ImageDraw.Draw(cm).ellipse(ellipse, fill=0)
        img.paste(Image.new("RGB", (r, r), BG), (ox, oy), cm)
...
def create_animated_progress_bar(
    ...
    try:
        fn_bold = ImageFont.truetype(FONT_BOLD, 15)
        fn_disc = ImageFont.truetype(FONT_REG,  11)
        fn_lbl  = ImageFont.truetype(FONT_REG,  12)
        fn_pct  = ImageFont.truetype(FONT_BOLD, 13)
    except Exception:
        fn_bold = fn_disc = fn_lbl = fn_pct = ImageFont.load_default()
    ...
    for i in range(total_frames + 1):
        ...
        # Card bg
        _rrect(draw, 4, 4, W - 8, H - 8, 14, CARD)

        # Left accent
        _rrect(draw, 4, 4, 5, H - 8, 3, ACCENT)

        # Logo border ring
        bd = 3
        _rrect(draw, LOGO_X - bd, LOGO_Y - bd,
               LOGO_SIZE + bd*2, LOGO_SIZE + bd*2,
               (LOGO_SIZE + bd*2) // 2, PURP_D)
        img.paste(logo, (LOGO_X, LOGO_Y), logo)

        # Player name
        draw.text((TEXT_X, 20), player_name, fill=WHITE, font=fn_bold)

        # Discord name
        if discord_name:
            draw.text((TEXT_X, 40), f"@{discord_name}", fill=GREY, font=fn_disc)

        # Divider
        draw.rectangle([TEXT_X, 60, W - PAD, 61], fill=DIV)

        # ── DKP ──
        curr_dkp = dkp_final * p
        pct_dkp  = int(curr_dkp)

        draw.text((TEXT_X, BAR_Y1 - 18), "DKP Progress", fill=GREY, font=fn_lbl)
        pct_str = f"{pct_dkp}%"
        pw = draw.textlength(pct_str, font=fn_pct)
        draw.text((W - PAD - pw, BAR_Y1 - 18), pct_str, fill=PURPLE, font=fn_pct)

        _rrect(draw, BAR_X, BAR_Y1, BAR_W, BAR_H, BAR_R, BAR_BG)
        fw = int(BAR_W * min(curr_dkp, 100) / 100)
        _grad_bar(img, draw, BAR_X, BAR_Y1, fw, BAR_H, BAR_R, PURP_D, PURPLE)
        if fw > 14:
            draw.rectangle([BAR_X + 7, BAR_Y1 + 3, BAR_X + fw - 6, BAR_Y1 + 5],
                           fill=(255, 255, 255, 50))
        ...
        draw.text((TEXT_X, BAR_Y2 - 18), "Deads Progress", fill=GREY, font=fn_lbl)
        pct_str2 = f"{pct_dead}%"
        pw2 = draw.textlength(pct_str2, font=fn_pct)
        draw.text((W - PAD - pw2, BAR_Y2 - 18), pct_str2, fill=RED, font=fn_pct)

        _rrect(draw, BAR_X, BAR_Y2, BAR_W, BAR_H, BAR_R, BAR_BG)
        fw2 = int(BAR_W * min(curr_dead, 100) / 100)
        _grad_bar(img, draw, BAR_X, BAR_Y2, fw2, BAR_H, BAR_R, RED_D, RED)
        if fw2 > 14:
            draw.rectangle([BAR_X + 7, BAR_Y2 + 3, BAR_X + fw2 - 6, BAR_Y2 + 5],
                           fill=(255, 255, 255, 50))
