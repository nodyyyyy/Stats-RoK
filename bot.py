...
@bot.tree.command(name="link", description="Link your Discord account to your RoK player ID")
...
@bot.tree.command(name="unlink", description="Unlink your Discord account from your RoK player ID")
...
@bot.tree.command(name="link_filler", description="Link a filler account to your main by its RoK player ID")
...
@bot.tree.command(name="unlink_filler", description="Remove a filler account linked to your main")
...
KVK_ADMIN_ROLE_ID = 1323254901022654470

@bot.tree.command(name="kvk", description="Activate or deactivate KvK mode (admins only)")
async def kvk(interaction: discord.Interaction):
    if not any(r.id == KVK_ADMIN_ROLE_ID for r in interaction.user.roles):
        await interaction.response.send_message("❌ You don't have permission to use this command.", ephemeral=True)
        return

    select = discord.ui.Select(
        placeholder="Select KvK status...",
        options=[
            discord.SelectOption(label="Activate KvK",   value="on",  emoji="⚔️",  description="Enable KvK mode for all members"),
            discord.SelectOption(label="Deactivate KvK", value="off", emoji="🛑", description="Disable KvK mode"),
        ]
    )

    async def select_callback(select_interaction: discord.Interaction):
        global KVK_ACTIVE
        if select_interaction.data["values"][0] == "on":
            KVK_ACTIVE = True
            await select_interaction.response.edit_message(
                content="✅ **KvK activated.** Members can now use `/my_stats`.",
                view=None
            )
        else:
            KVK_ACTIVE = False
            await select_interaction.response.edit_message(
                content="🛑 **KvK deactivated.**",
                view=None
            )
    ...
    status_text = "🟢 Currently **ON**" if KVK_ACTIVE else "🔴 Currently **OFF**"
    await interaction.response.send_message(
        f"**KvK Status** — {status_text}\nSelect a new status below:",
        view=view,
        ephemeral=True
    )
...
@bot.tree.command(name="my_stats", description="View your personal KvK statistics and progress")
...
@bot.tree.command(name="req", description="View your KvK requirements for DKP and deaths")
