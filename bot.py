...
def create_animated_progress_bar(
    ...
    W, H   = 700, 220
    BG     = (12,12,20); CARD=(18,18,30); BAR_BG=(35,35,55)
    PURPLE = (155,89,255); PURP_D=(70,25,140)
    RED    = (231,76,60);  RED_D=(110,18,18)
    WHITE  = (255,255,255); GREY=(140,140,165)
    DIV    = (40,40,62);   ACCENT=(123,44,191)

    # DejaVu supports Unicode (ø, å, ñ, etc.)
    try:
        fn_name = ImageFont.truetype(FONT_BOLD, 24)
        fn_disc = ImageFont.truetype(FONT_REG,  18)
        fn_lbl  = ImageFont.truetype(FONT_REG,  18)
        fn_pct  = ImageFont.truetype(FONT_BOLD, 19)
    except Exception:
        fn_name = fn_disc = fn_lbl = fn_pct = ImageFont.load_default()
    ...
    for i in range(total_frames + 1):
        ...
        # ── Card bg ──
        draw.rectangle([4*SCALE, 4*SCALE, (W-4)*SCALE, (H-4)*SCALE], fill=CARD)

        # ── Left accent ──
        draw.rectangle([4*SCALE, 4*SCALE, 9*SCALE, (H-4)*SCALE], fill=ACCENT)

        # ── Logo border ring ──
        bd = 3
        lx, ly = LOGO_X * SCALE, LOGO_Y * SCALE
        ls = LOGO_SIZE * SCALE
        draw.ellipse([lx - bd*SCALE, ly - bd*SCALE,
                      lx + ls + bd*SCALE, ly + ls + bd*SCALE], fill=PURP_D)

        # Paste logo scaled up
        logo_big = _load_logo(ls)
        img.paste(logo_big, (lx, ly), logo_big)

        # ── Fonts scaled up ──
        try:
            fn_name_s = ImageFont.truetype(FONT_BOLD, 24 * SCALE)
            fn_disc_s = ImageFont.truetype(FONT_REG,  18 * SCALE)
            fn_lbl_s  = ImageFont.truetype(FONT_REG,  18 * SCALE)
            fn_pct_s  = ImageFont.truetype(FONT_BOLD, 19 * SCALE)
        except Exception:
            fn_name_s = fn_disc_s = fn_lbl_s = fn_pct_s = ImageFont.load_default()

        tx = TEXT_X * SCALE

        # ── Player name ──
        draw.text((tx, 20*SCALE), player_name, fill=WHITE, font=fn_name_s)

        # ── Discord name ──
        if discord_name:
            draw.text((tx, 42*SCALE), f"@{discord_name}", fill=GREY, font=fn_disc_s)

        # ── Divider ──
        draw.rectangle([tx, 62*SCALE, (W-PAD)*SCALE, 63*SCALE], fill=DIV)

        # ── DKP bar ──
        curr_dkp = dkp_final * p
        pct_dkp  = int(curr_dkp)

        bx, by1 = BAR_X*SCALE, BAR_Y1*SCALE
        bw, bh  = BAR_W*SCALE, BAR_H*SCALE

        draw.text((tx, (BAR_Y1-20)*SCALE), "DKP Progress", fill=GREY, font=fn_lbl_s)

        pct_str = f"{pct_dkp}%"
        pw = draw.textlength(pct_str, font=fn_pct_s)
        draw.text(((W-PAD)*SCALE - pw, (BAR_Y1-20)*SCALE), pct_str, fill=PURPLE, font=fn_pct_s)

        # bg bar (rectangular)
        draw.rectangle([bx, by1, bx+bw, by1+bh], fill=BAR_BG)

        # fill bar with gradient
        fw = int(bw * min(curr_dkp, 100) / 100)
        if fw > 0:
            for col_i in range(fw):
                t = col_i / max(fw - 1, 1)
                col = tuple(int(PURP_D[k] + (PURPLE[k] - PURP_D[k]) * t) for k in range(3))
                draw.rectangle([bx + col_i, by1, bx + col_i, by1 + bh], fill=col)
        ...
        draw.text((tx, (BAR_Y2-20)*SCALE), "Deads Progress", fill=GREY, font=fn_lbl_s)

        pct_str2 = f"{pct_dead}%"
        pw2 = draw.textlength(pct_str2, font=fn_pct_s)
        draw.text(((W-PAD)*SCALE - pw2, (BAR_Y2-20)*SCALE), pct_str2, fill=RED, font=fn_pct_s)

        draw.rectangle([bx, by2, bx+bw, by2+bh], fill=BAR_BG)

        fw2 = int(bw * min(curr_dead, 100) / 100)
        if fw2 > 0:
            for col_i in range(fw2):
                t = col_i / max(fw2 - 1, 1)
                col = tuple(int(RED_D[k] + (RED[k] - RED_D[k]) * t) for k in range(3))
                draw.rectangle([bx + col_i, by2, bx + col_i, by2 + bh], fill=col)

        # Downscale 2x → anti-aliased result
    ...
    frames[0].save(
        ...
        loop=1,        # play once then stop (0 = infinite)
        optimize=False,
    )
    buffer.seek(0)
    return buffer
