...
def blocking_refresh_cache():
    ...
    try:
        ...
        new_cache["Links"] = pd.DataFrame(records) if records else pd.DataFrame(columns=headers)
        ...
        if STATS_SHEET_ID:
            ...
            for ws in stats_spreadsheet.worksheets():
                ...
                new_cache[ws.title] = pd.DataFrame(records) if records else pd.DataFrame(columns=headers)
...
async def link(interaction: discord.Interaction, rok_id: str):
    ...
    if "Discord ID" in df.columns and str(interaction.user.id) in df["Discord ID"].astype(str).values:
...
async def my_stats(interaction: discord.Interaction):
    ...
    if any(s.lower() == "overall" for s in all_stat_sheets): ordered_sheets.append("Overall")
    ...
    if overall_df is not None:
        ...
        if not m_row.empty:
            r = m_row.iloc[0]
            main_name, main_power = r.get('Name', 'Unknown'), clean_number(r.get('Power', 0))
            main_current_power = clean_number(r.get('Current Power', 0))
            dkp, goal_dkp = clean_number(r.get('DKP', 0)), clean_number(r.get('Goal DKP', 1))
            deads = clean_number(r.get('Deads', 0))
            req_deads = clean_number(r.get('Required Deads', r.get('Requiered Deads', 1)))
    ...
    if overall_df is not None and flinks:
        for fid in flinks:
            ...
            if not row_f.empty:
                f = row_f.iloc[0]
                fn, p, df_ = f.get("Name", "Unknown"), clean_number(f.get("Initial Power", f.get("Power", 0))), clean_number(f.get("Deads", 0))
