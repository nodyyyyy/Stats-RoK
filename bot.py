...
def create_stats_card(dkp_pct=0, dead_pct=0, player_name="Player", discord_name=""):
    ...
    def draw_bar(y_base, label, pct, color, dark):
        y = y_base * SCALE
        ...
        for ci in range(fw):
            t = ci / max(fw - 1, 1)
            col = tuple(int(dark[k] + (color[k] - dark[k]) * t) for k in range(3)) + (255,)
            gd.rectangle([BAR_X+ci, y, BAR_X+ci, y+BAR_H], fill=col)
        mask_l = Image.new("L", (iW, iH), 0)
        ImageDraw.Draw(mask_l).rounded_rectangle(
            [BAR_X, y, BAR_X+fw, y+BAR_H], radius=BAR_R, fill=255)
        base.paste(grad, mask=mask_l)
        if fw > 30*SCALE:
            shimmer = Image.new("RGBA", (iW, iH), (0, 0, 0, 0))
            ImageDraw.Draw(shimmer).rounded_rectangle(
                [BAR_X + 8*SCALE, y + 3*SCALE, BAR_X + fw - 8*SCALE, y + 7*SCALE],
                radius=3*SCALE, fill=(255, 255, 255, 55))
            base.paste(Image.alpha_composite(base, shimmer))

    draw_bar(96,  "DKP Progress",   dkp_pct,  PURPLE, PURP_D)
    draw_bar(148, "Deads Progress", dead_pct, RED,    RED_D)
